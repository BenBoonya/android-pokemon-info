...
class BerryListViewModel(private val getPokemonListUseCase: GetPagedListUseCase) : ViewModel() {

    private val pagedListResult: MutableLiveData<PagedListResult<GenericListItem>> = MutableLiveData()

    val berryList: LiveData<PagedList<GenericListItem>> = Transformations.switchMap(pagedListResult) { it.result }
    ...
    fun getBerryList() {
        pagedListResult.value = getPokemonListUseCase("berry")
    }
}